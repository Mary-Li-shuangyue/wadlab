...
public class StringList {
	...
	// <6> we iterate until we hit end of list or encounter an element greater
	// than s
	// and then we need to insert s in that position
	// <9> we add a new element after the cursor, with next as new element's
	// "tail"
	...
	public boolean equal(StringList l) {
		Node cursor = head;
		while (cursor != null) {
			if (!(cursor.getNext().equals(l.head.getNext()))) {
				return false;
			}
		}
		return true;
	...
	public StringList reverse() {
		...
		while (cursor != null) {
			temp.head = new Node(cursor.getElement(), temp.head);
			cursor = cursor.getNext();
		}
		return temp;
	}
